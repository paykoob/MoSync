...
const SParseOpt KSISTypes[] =
	{
		...
		{L"SC", EInstSISConfig},
		{L"SP", EInstSISPatch},
		{L"SU", EInstSISUpgrade},
		{L"SISAPP",		EInstSISApp},
		{L"SISSYSTEM",  EInstSISSystem},
		{L"SISOPTION",	EInstSISOption},
		{L"SISCONFIG",	EInstSISConfig},
		{L"SISPATCH",	EInstSISPatch},
		{L"SISUPGRADE", EInstSISUpgrade},
	};
...
const SParseOpt KRunOptions[] =
	{
		{L"RI", EInstFileRunOptionInstallOnly},
		{L"RR", EInstFileRunOptionRemoveOnly},
		{L"RB", EInstFileRunOptionInstallAndRemove},
		{L"RUNINSTALL",	EInstFileRunOptionInstallOnly},
		{L"RUNREMOVE",	EInstFileRunOptionRemoveOnly},
		{L"RUNBOTH",	EInstFileRunOptionInstallAndRemove}
	};
...
// Returns  : Success or failure (actually, always TRUE - failure causes an exception to be thrown)
	...
	if ((wType==EInstSISConfig) || (wType==EInstSISUpgrade) || (wType==EInstSISPatch))
		{
		...
		if (m_token==',')
			...
			switch (pNode->file->type)
				...
				case EInstFileTypeRun:
					...
					if (options&(EInstFileRunOptionRemoveOnly|EInstFileRunOptionInstallAndRemove) &&
						!(options&EInstRunWaitEnd))
...
void CParsePkg::ParseSignatureL()
{
	...
	m_pObserver->DoErrMsg(__T("digital signatures not supported in this version of makesis"));
#endif
}
				...
				case ALPHA_TOKEN:
					{
						...
						if(!wcsnicmp(m_tokenValue.pszString, L"option",6))
							...
							expr->pPrim->dwNumber = EVarOptionBase+optionNum;
							}
						else
							{
