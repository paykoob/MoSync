...
namespace avmplus
{
	...
	// added by niklas (mobile sorcery)

	const char *pipe_regs[] = {
			"REG_zero",
			"REG_sp",
			"REG_rt",
			"REG_fr",
	...
	void PrintWriter::format(const char *str, ...) {
		if(bufferpos == NULL) { bufferpos = buffer; *bufferpos = NULL; }
		va_list vl;
		va_start(vl,str);
		char outString[1024];
		char *dst = outString;
		*dst = 0;
		while(1) {
			...
			switch(*str) {
				...
				case 'd':
						dst+=sprintf(dst, "%d", va_arg(vl,int));
					break;
			}
			str++;
		}
		...
		memcpy(bufferpos, outString, len);
		bufferpos += len;
	}
...
#endif
		conditionCode = AL;
		//immediatePool = new MDInstruction[kImmediatePoolMax];
		immediatePool = 0;
		immediatePoolCount = 0;
		kImmediatePoolMax = 1024;
	}
...
#define PRINT_LAST_INSTRUCTION()
	...
	void ArmAssembler::MVN_imm8(Register dst, int imm8) {
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag)
			console->format("    %A  mvn%s   %R, %d\n", mip, conditionCodes[conditionCode], dst, imm8);
#endif /* AVMPLUS_VERBOSE */
...
// added by mobile sorcery (niklas)
	void ArmAssembler::BX(Register dst)
	{
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag)
			console->format("    %A  bx%s     %R\n", mip, conditionCodes[conditionCode], dst);
#endif /* AVMPLUS_VERBOSE */
		*mip++ = 0x012FF10 | (conditionCode<<28) | dst;
		PRINT_LAST_INSTRUCTION();
	}

//-----------------------------------------------------

	void ArmAssembler::LDR(Register dst, int offset, Register base, bool writeBack )
	{
		int off = offset;
		int opcode;
		if (off<0) {
		...
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (off&0xFFF) | (((int)writeBack)<<21);
		PRINT_LAST_INSTRUCTION();
	}
...
// added by mobile sorcery (niklas)
	...
	void ArmAssembler::LDRHS(Register dst, int offset, Register base)
	{
		int opcode = 0x1D000F0;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  ldrhs%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  ldrhs%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  ldrhs%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0x0F) | ((offset&0xF0)<<4);
	}

	void ArmAssembler::LDRBS(Register dst, int offset, Register base)
	{
		int opcode = 0x1D000D0;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  ldrbs%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  ldrbs%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  ldrbs%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0x0F) | ((offset&0xF0)<<4);
	...
	void ArmAssembler::LDRH(Register dst, int offset, Register base)
	{
		int opcode = 0x005000B0;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  ldrh%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  ldrh%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  ldrh%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0x0F) | ((offset&0xF0)<<4);
	...
	void ArmAssembler::STRH(Register dst, int offset, Register base)
	{
		int opcode = 0x004000B0;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  strh%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  strh%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  strh%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0x0F) | ((offset&0xF0)<<4);
	...
	void ArmAssembler::STRB(Register dst, int offset, Register base)
	{
		int opcode = 0x4400000;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  strb%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  strb%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  strb%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0xfff);
	}

	void ArmAssembler::LDRB(Register dst, int offset, Register base)
	{
		int opcode = 0x4500000;
		incInstructionCount();
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			if (base == PC) {
				console->format("    %A  ldrb%s   %R, %d(%R) // %A\n",
					mip, conditionCodes[conditionCode], dst, offset, base, offset+8+(int)mip);
			} else if (opcode & 0x02000000) {
				console->format("    %A  ldrb%s   %R, %R(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			} else {
				console->format("    %A  ldrb%s   %R, %d(%R)\n", mip, conditionCodes[conditionCode], dst, offset, base);
			}
		}
#endif /* AVMPLUS_VERBOSE */
		*mip++ = opcode | (conditionCode<<28) | (base<<16) | (dst<<12) | (offset&0xfff);
...
//-----------------------------------------------------
	...
	void ArmAssembler::MOV_imm16(Register dst, int imm16, bool optimize)
	{
		AvmAssert((imm16 & 0xFFFF0000) == 0);

		MOV_imm8 (dst, imm16 & 0xFF);

		if (optimize && !(imm16 & 0xFF00)) return;
		ADD_imm8_hi (dst, dst, (imm16 >> 8) & 0xFF);
	}

	void ArmAssembler::ADD_imm16(Register dst, Register src, int imm16, bool optimize)
	{
		AvmAssert((imm16 & 0xFFFF0000) == 0);
	...
	/*
	void ArmAssembler::MOV_imm32(Register dst, int imm32, bool optimize)
	{
...
#ifdef AVMPLUS_VERBOSE
		if (verboseFlag) {
			const char *reg = findPipeReg(imm32);
			if(reg) {
				console->format("    // begin %s\n", reg);
			}

			console->format("    mov %R, %d // imm32\n", dst, imm32);
		}
#endif
		if(!optimize) {
			MOV_imm16(dst, (imm32&0xffff0000)>>16);
			LSL_i(dst, dst, 16);
			ADD_imm16(dst, dst, (imm32&0x0000ffff));
		} else {
			if(imm32&0xffff0000) {
				MOV_imm16(dst, (imm32&0xffff0000)>>16, true);
				LSL_i(dst, dst, 16);
				if(imm32&0x0000ffff)
					ADD_imm16(dst, dst, (imm32&0x0000ffff), true);
			} else {
				MOV_imm16(dst, (imm32&0x0000ffff), true);
			}
		}


		// this is slightly faster on the phone than the commented version
		//LDR(dst, -8+8, AA::PC); // -8+8 just to show what is done :)
		//ConditionCode lastCode = this->conditionCode;
		//SET_CONDITION_CODE(AL);
		//B(8);
		//SET_CONDITION_CODE(lastCode);
		//IMM32(imm32);
		//incInstructionCount();
...
#define SCRATCH_SIZE 32768
	...
	void* CodegenMIR::emitImtThunk(ImtBuilder::ImtEntry *e)
	{
		...
		(e->next != NULL); // must have 2 or more entries
